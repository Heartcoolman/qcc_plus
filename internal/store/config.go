...
func (s *Store) LoadAllByAccount(ctx context.Context, accountID string) (records []NodeRecord, cfg Config, activeID string, err error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&r.ID, &r.Name, &r.BaseURL, &r.APIKey, &r.HealthCheckMethod, &r.AccountID, &r.Weight, &r.SortOrder, &r.Failed, &r.Disabled, &r.LastError, &r.CreatedAt, &r.Requests, &r.FailCount, &r.FailStreak, &r.TotalBytes, &r.TotalInput, &r.TotalOutput, &r.StreamDurMs, &r.FirstByteMs, &r.LastPingMs, &r.LastPingErr, &lastHealthAt)
		if err != nil {
			return
		}
